...
| 146 | 2026-08-21 | `.agents/references/routing.md` maps BytesNation to `effort/bytesnation` and appends sub-ventures (`effort/keepstar-ventures/cheqpass`), which contradicts 119 and would produce `effort/bytesnation/capstan`. That file states it wins over an agent prompt. The exception is documented nowhere | open |
...
| 144 | 2026-08-21 | The five pre-existing notes carrying `status/done` are the operator's call, not this effort's. They predate it and mass-editing committed vault notes is not a delivery step | accepted |
...
| 142 | 2026-08-21 | `status/done` is wrong for a note under `Efforts/Active/`. The vault's `schemas.md` makes the Efforts folder the lifecycle authority and `vault-health.py` errors on anything but `status/active` there. Corrected by a second commit, per 127 | accepted |
...
| 119 | 2026-08-21 | Supersedes 113. An `effort/*` tag names the project, not the individual effort. Confirmed on a second folder: `Efforts/Active/Keepstar Ventures/` holds 213 notes across ten project subfolders, every one carrying the parent tag | accepted |
...
| 113 | 2026-08-21 | An `effort/*` tag names the project, not the individual effort. Counted: 114, 103 and 74 notes on the three commonest, against one each on the two I first cited as the convention. My diagnosis was wrong; the Courier filed a project under its parent organisation | superseded by 119 |
